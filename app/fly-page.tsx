...
export default function FlyPage(){
 ...
 return <main className={`fly-studio ${detail?'has-selection':''}`}>
  ...
  {s.mode!=='fly'&&<aside className={`structure-list ${listOpen?'open':''}`} aria-label="Explore structures">
   ...
   <button className="all-structures" onClick={()=>{s.mode==='cells'?setCells(emptyCells):patch({visible:GROUPS.map(g=>g.id),selected:null,isolate:false,reset:s.reset+1});}}> {s.mode==='cells'?'Whole view':'All regions'}<ChevronRight size={13}/></button>
