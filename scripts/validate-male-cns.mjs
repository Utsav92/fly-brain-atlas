...
assert.equal(cells.neurons.length,1712);assert.equal(new Set(cells.neurons.map(n=>n.id)).size,1712);assert.equal(cells.featured.length,8);
...
assert.equal(cells.neurons.reduce((s,p)=>s+p.edges,0),2180126);assert.equal(cells.neurons.filter(n=>n.mesh).length,8);
...
console.log('PASS: 80 annotated, paired regions; 7,270,902 triangles; 1712 skeletons; 2,180,126 edges; 8 native neuron surfaces; source hashes and valid geometry; 69 body components.');
